...
<center><h4>Laporan Pengembalian Perbulan<br>Bulan : <?php echo $bulan2[$bulan1]  ?> <?php echo $tahun1 ?></h3></h4></center>
    <table class="table table-bordered table-striped" border="1" align="center" style="width: 65%;" cellpadding="5" cellspacing="0">
        ...
        <?php
            $no = 1;
              $totalBayar = 0;
            $query = mysqli_query($koneksi, "SELECT *,sum(jml_item) as jml FROM tbl_pengembalian where month(tanggal_kembali)='$bulan1' and status='DI KEMBALIKAN' GROUP by day(tanggal_kembali)");
            while ($data = mysqli_fetch_array($query)) {
                ?>

                <tbody>
                    <td><?= $no++ ?></td>
                    <td><?php echo date('d-F-Y', strtotime($data['tanggal_kembali'])) ?></td>
