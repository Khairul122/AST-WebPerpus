...
<center><h4>Laporan Peminjaman Pertahun <br>Tahun : <?php echo $tahun ?></h3></h4></center>
 <table class="table table-bordered table-striped" border="1" align="center" style="width: 65%;" cellpadding="5" cellspacing="0">
        ...
        <?php
            $no = 1;
             $totalBayar = 0;
            $query = mysqli_query($koneksi, "SELECT *, sum(jumlah) as jm, MONTH(tgl_pinjam)AS bln FROM tbl_peminjaman where year(tgl_pinjam)='$tahun' and status='Di Pinjam'  GROUP by month(tgl_pinjam)");
